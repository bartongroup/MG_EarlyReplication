...
plot_sel_peak_growth <- function(pg, n_peaks, seed = 42) {
  set.seed(seed)
  sel_peaks <- pg |>
    count(peak_id) |>
    filter(n == 8) |> 
...
plot_growth <- function(g_test, ylab1, ylab2, point_size = 2, cex = 2, ymin = NA_real_, ymax = NA_real_) {
  g1 <- g_test$lm_fit |> 
    arrange(peak_id) |> 
    ...
    geom_hline(yintercept = 2, colour = "orange", linetype = "dashed") +
    geom_point() +
    geom_errorbar() +
